...
def fake_load_positions(username):
    ...
    time.sleep(3)  # Simulate 5 seconds of processing time
    ...
    r.set(f"{username}_positions", json.dumps(positions))
    r.set(f"{username}_status", "COMPLETE")
...
def start_loading():
    ...
    # Set initial status
    r.set(f"{username}_status", "PENDING")
...
def get_position():
    ...
    # Check status
    status = r.get(f"{username}_status")
    if status is None:
        return jsonify({"message": "No task found for this username"}), 404

    status = status.decode('utf-8')
    if status == "PENDING":
        return jsonify({"message": "Positions are still loading"}), 202

    # Positions are ready, get them
    positions_json = r.get(f"{username}_positions")
    if positions_json:
        positions = json.loads(positions_json)
        position = positions.pop(0)  # Get the first position
        if positions:
            r.set(f"{username}_positions", json.dumps(positions))  # Save remaining positions
        else:
            r.delete(f"{username}_positions")  # No more positions, clean up
            r.delete(f"{username}_status")  # Clean up status
